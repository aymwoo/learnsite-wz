...
public partial class Student_myevaluate : System.Web.UI.Page
{
    ...
    protected void DataListvote_ItemCommand(object source, DataListCommandEventArgs e)
    {
        ...
        if (e.CommandName == "S")
        {
                ...
                if (israuge)
                    Literal1.Text = LearnSite.Common.ViewPage.SelectWritePlugin(Wid, ext, Wurl, wmodel.Wcode,wmodel.Wthumbnail, true, false,wmodel.Wnum);
            ...
            if (Request.QueryString["mid"] != null)
            {
                if (DataListGauge.Items.Count > 0)
                {
                    ...
                    if (myselect.CheckCount > 0)
                    {
                        ...
                        int Mygrade = cook.Sgrade;
                        int Myclass = cook.Sclass;
    ...
    {
        string Studentnum = cook.Snum;

        int MySgrade = cook.Sgrade;
        int MySclass = cook.Sclass;
        int MySyear = cook.Syear;
        int MySgroup = 0;// cook.Sgroup;
        if (MySgroup == 0)
            Labelscope.Text = "全班";
        else
            Labelscope.Text = "组内";
        ...
        if (LearnSite.Common.WordProcess.IsNum(Mid))
        {
            ...
            if (wmodel != null)
            {
                DataListvote.DataSource = ws.ShowMissionWorks(MySyear,MySgrade, MySclass, Int32.Parse(Mid));
                DataListvote.DataBind();
