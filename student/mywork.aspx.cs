...
public partial class Student_mywork : System.Web.UI.Page
{
    ...
    private void ShowWorksTop()
    {
        ...
        Topwork.DataSource = ws.ShowBestWork(cook.Sgrade, cook.Syear, cook.ThisTerm);
        Topwork.DataBind();
    }
    ...
    protected void Topwork_RowDataBound(object sender, GridViewRowEventArgs e)
    {        
        if (e.Row.RowIndex > -1)
        {
            e.Row.Cells[0].Text = Convert.ToString(Topwork.PageIndex * Topwork.PageSize + e.Row.RowIndex + 1);
        }
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            //当鼠标放上去的时候 先保存当前行的背景颜色 并给附一颜色 
