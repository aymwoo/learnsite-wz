...
public partial class Student_quizstart : System.Web.UI.Page
{
            ...
            if (qmodel.Qopen.Value)
            {
                ...
                if (!string.IsNullOrEmpty(selectclass))
                {
                    ...
                    if (isok)
                    {
                        ...
                        {
                            Session["quizQanswer"] = "false";
                        }
                        else
                        {
                            Session["quizQanswer"] = qmodel.Qanswer.ToString();
                        }
                    }
    ...
    private void finished()
    {
        Session["quizRight"] = "-1";
        Session["quizWrong"] = "-1";
        int allscore = CalculateOnly() + CalculateMore() + CalculateJudge();
        Labelallscore.Text = allscore.ToString();

        if (allscore > 0)
        {
            string Quizrights = Session["quizRight"].ToString();
            string Quizwrongs = Session["quizWrong"].ToString();
            string Quizqids = Quizrights + "," + Quizwrongs;
            Session["quizRightWrong"] = Quizqids;
    ...
    private int CalculateOnly()
    {
        ...
        foreach (DataListItem item in this.DataListonly.Items)
        {
            ...
            {
                allscore += thisscore;
                Session["quizRight"] = Session["quizRight"].ToString() + "," + DataListonly.DataKeys[item.ItemIndex].ToString();
            }
            else
            {
                wrong++;
                Session["quizWrong"] = Session["quizWrong"].ToString() + "," + DataListonly.DataKeys[item.ItemIndex].ToString();
            }
        }
        //Labelonly.Text = "得分" + allscore.ToString() + " 有" + wrong + "题错误";
        return allscore;
    }
    private int CalculateMore()
    {
        int allscore = 0;
        int wrong = 0;
        foreach (DataListItem item in this.DataListmore.Items)
        {
            ...
            {
                getscore = thisscore;
                Session["quizRight"] = Session["quizRight"].ToString() + "," + DataListmore.DataKeys[item.ItemIndex].ToString();
            ...
            else
            {
                ...
                wrong++;
                Session["quizWrong"] = Session["quizWrong"].ToString() + "," + DataListmore.DataKeys[item.ItemIndex].ToString();
    ...
    private int CalculateJudge()
    {
        ...
        foreach (DataListItem item in this.DataListjudge.Items)
        {
            ...
            {
                allscore += thisscore;
                Session["quizRight"] = Session["quizRight"].ToString() + "," + DataListjudge.DataKeys[item.ItemIndex].ToString();

            }
            else
            {
                wrong++;
                Session["quizWrong"] = Session["quizWrong"].ToString() + "," + DataListjudge.DataKeys[item.ItemIndex].ToString();
            }
        }
    ...
    protected void Btnquiz_Click(object sender, EventArgs e)
    {
        ...
        if (strscore != "")
        {
            ...
            if (Rscore > 0)
            {
                ...
                string Rwrong = "";
                string Rhistory = "";
                if (Session["quizRightWrong"] != null)
                {
                    Rwrong = Session["quizWrong"].ToString();
                    Rhistory = Session["quizRightWrong"].ToString();
                }
                ...
                if (bll.ExistsBynumdate(Rsid, Rdate))
                {
                        ...
                        {
                            Labelmsg.Text = "恭喜成绩进步，已更新！";
                        }
                        else
                        {
                            Labelmsg.Text = "比上次落后，不作更新！";
