...
namespace LearnSite.DAL
{
	...
	public class Computers
	{
        ...
        /// <summary>
        /// 将当前上课班级学号自动分配给当前机房IP列表，方便培训课时，自动填充登录学号，以免重复
        /// </summary>
        /// <param name="hid"></param>
        public void AutoAssign(string hid) {
            string mysql = "select Sgrade,Sclass,Snum from Students,Room where Sgrade=Rgrade and Sclass=Rclass and Rset=1 and Rhid=" + hid;
            DataTable dt = DbHelperSQL.Query(mysql).Tables[0];
            int dcount = dt.Rows.Count;

            string sqlstr = "select Pip from Computers";
        ...
        {
            string sql = "update Computers set Pon=0, Pnum='' ";
            DbHelperSQL.ExecuteSql(sql);
        }
        /// <summary>
        /// 根据Ip获取分配的学号
        ...
        /// <param name="ip"></param>
        /// <returns></returns>
        public string getIpSnum(string ip)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Pnum from Computers where Pip=@Pip");
