...
namespace LearnSite.DAL
{
	...
	public class Room
	{
        ...
        public void AddRoom(int Rgrade,int Rclass)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Room(");
            strSql.Append("Rgrade,Rclass,RgroupMax) ");
            strSql.Append(" values (");
            strSql.Append("@Rgrade,@Rclass,@RgroupMax)");
            SqlParameter[] parameters = {
					new SqlParameter("@Rgrade", SqlDbType.Int,4),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 Rid,Rhid,Rgrade,Rclass,Rset,Rpwd,Rlock,Rip,Rgauge,Rclassedit,Rphotoedit,Rsexedit,Rnameedit,Rcid,Ropen,Rseat,Rshare,Rpwdsee,Rgroupshare,Rreg,Rscratch,Rlogin,Rpass from Room ");
            strSql.Append(" where Rgrade=@Rgrade and Rclass=@Rclass");
            SqlParameter[] parameters = {
					new SqlParameter("@Rgrade", SqlDbType.Int,4),
					new SqlParameter("@Rclass", SqlDbType.Int,4)};
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["Rpass"] != null && ds.Tables[0].Rows[0]["Rpass"].ToString() != "")
                {
                    if ((ds.Tables[0].Rows[0]["Rpass"].ToString() == "1") || (ds.Tables[0].Rows[0]["Rpass"].ToString().ToLower() == "true"))
                    {
                        model.Rpass = true;
                    }
                    else
        ...
        {
            string mysql = "select Rpass from Room where Rgrade=" + Rgrade + " and Rclass=" + Rclass;
        ...
        public void UpdateRpass(int Rgrade, int Rclass, bool Rpass)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Room set ");
            strSql.Append("Rpass=@Rpass");
            strSql.Append(" where Rgrade=@Rgrade and Rclass=@Rclass");
            SqlParameter[] parameters = {
					new SqlParameter("@Rgrade", SqlDbType.Int,4),
					new SqlParameter("@Rclass", SqlDbType.Int,4),
					new SqlParameter("@Rpass", SqlDbType.Bit,1)};
            parameters[0].Value = Rgrade;
            parameters[1].Value = Rclass;
            parameters[2].Value = Rpass;
        ...
        public void UpdateMyRlogin(int Rgrade, int Rclass, bool Rlogin)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Room set ");
            strSql.Append("Rlogin=@Rlogin");
            strSql.Append(" where Rgrade=@Rgrade and Rclass=@Rclass");
            SqlParameter[] parameters = {
					new SqlParameter("@Rgrade", SqlDbType.Int,4),
					new SqlParameter("@Rclass", SqlDbType.Int,4),
					new SqlParameter("@Rlogin", SqlDbType.Bit,1)};
            parameters[0].Value = Rgrade;
            parameters[1].Value = Rclass;
            parameters[2].Value = Rlogin;
        ...
        {
            string strSql = "select distinct Rgrade FROM Room Where Rset=1 order by Rgrade";            
            DataSet ds = DbHelperSQL.Query(strSql);
            int dscount = ds.Tables[0].Rows.Count;
            if (dscount == 0)
            {
                strSql = "select distinct Rgrade FROM Room  order by Rgrade";
        ...
        {
            string strSql = "select distinct Rclass FROM Room where Rset=1 order by Rclass";

            DataSet ds = DbHelperSQL.Query(strSql);
            int dscount = ds.Tables[0].Rows.Count;
            if (dscount == 0)
            {
                strSql = "select distinct Rclass FROM Room order by Rclass";
                ds = DbHelperSQL.Query(strSql);
                dscount = ds.Tables[0].Rows.Count;
                if (dscount == 0)
                {
        ...
        {
            string strSql = "select distinct Rclass FROM Room order by Rclass asc ";
            DataSet ds = DbHelperSQL.Query(strSql);
            int dscount = ds.Tables[0].Rows.Count;
            if (dscount == 0)
            {
        ...
        {
            string mysql = "select Rid from Room where Rgrade=" + Rgrade + " and Rclass=" + Rclass;
