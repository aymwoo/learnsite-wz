...
namespace LearnSite.DAL
{
	...
	public partial class SurveyFeedback
	{
		...
		public int Add(LearnSite.Model.SurveyFeedback model)
		{
			...
			strSql.Append("Fnum,Fyear,Fgrade,Fclass,Fterm,Fcid,Fvid,Fvtype,Fselect,Fscore,Fdate,Fsid,Flid)");
            ...
            parameters[12].Value = model.Flid;
        ...
        /// <param name="Fvid"></param>
        /// <returns></returns>
        public string GetClassFselect(int Fyear, int Fgrade, int Fclass, int Fvid)
        {
            string allselect = "";
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Fselect ");
            strSql.Append(" FROM SurveyFeedback ");
            strSql.Append(" where Fvid=@Fvid  and  Fyear=@Fyear and Fgrade=@Fgrade and Fclass=@Fclass and Fsid>0 ");
        ...
        public int GetClassYscore(int Fyear, int Fgrade, int Fclass, int Fterm, int Fvid)
        {
            ...
            strSql.Append(" FROM SurveyFeedback ");
            strSql.Append(" where Fsid>0 and Fyear=@Fyear and Fgrade=@Fgrade and Fclass=@Fclass and Fterm=@Fterm and Fvid=@Fvid ");
            SqlParameter[] parameters = {
					new SqlParameter("@Fyear", SqlDbType.Int,4),
					new SqlParameter("@Fgrade", SqlDbType.Int,4),
					new SqlParameter("@Fclass", SqlDbType.Int,4),
        ...
        public string ShowStuFeedbackCids(string Snum,int Fterm,int Fgrade)
        {
            ...
            if (n > 0)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    cidstr = dt.Rows[i]["Fcid"].ToString();
