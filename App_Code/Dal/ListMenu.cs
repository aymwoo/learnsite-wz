...
namespace LearnSite.DAL
{
	...
	public partial class ListMenu
	{
        ...
        /// </summary>
        /// <param name="Lcid"></param>
        public void Lsortsncy(int Lcid)
        {
            string mysql = "update Mission set Msort=Lsort from Mission,ListMenu where Mcid=Lcid and Mid=Lxid and Msort!=Lsort and Mdelete=0 "; 
        ...
        public void UpdateLshow(int Lcid,int Lxid)
        {
            ...
            string mysql = " update Mission set Mpublish=Lshow from Mission,ListMenu where Mcid=Lcid and Mid=Lxid and Mcid=" + Lcid + " and Mid=" + Lxid;
            DbHelperSQL.ExecuteSql(mysql);
        }
        public void UpdateLshow(int Lid)
        {
            ...
            string mysql = " update Mission set Mpublish=Lshow from Mission,ListMenu where Mcid=Lcid and Mid=Lxid and Lid=" + Lid;
            DbHelperSQL.ExecuteSql(mysql);
        }
        ...
        /// <summary>
        /// 关联更新一条数据 
        /// 条件Lcid,Lxid,oldLtype 更新Lshow,Ltitle,Ltype
        /// </summary>
        public bool UpdateMenuThree(LearnSite.Model.ListMenu model,int oldLtype)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ListMenu set ");
            strSql.Append("Lshow=@Lshow,");
            strSql.Append("Ltitle=@Ltitle,");
            strSql.Append("Ltype=@Ltype");
            strSql.Append(" where Lcid=@Lcid and Lxid=@Lxid and Ltype=@oldLtype");
            SqlParameter[] parameters = {
					new SqlParameter("@Lcid", SqlDbType.Int,4),
					new SqlParameter("@Ltype", SqlDbType.Int,4),
					new SqlParameter("@Lxid", SqlDbType.Int,4),
					new SqlParameter("@Lshow", SqlDbType.Bit,1),
					new SqlParameter("@Ltitle", SqlDbType.NVarChar,50),
					new SqlParameter("@oldLtype", SqlDbType.Int,4)};
            parameters[0].Value = model.Lcid;
            parameters[1].Value = model.Ltype;
            parameters[2].Value = model.Lxid;
            parameters[3].Value = model.Lshow;
            parameters[4].Value = model.Ltitle;
            parameters[5].Value = oldLtype;
        ...
        /// <summary>
        /// 将导入学案生成新导航,共用Mission表的任务列表
        /// </summary>
        public void importmenu(int Cid)
        {
            // 类型：活动1，调查2，讨论3，表单４，积木编程5，阅读6,Python编程8,测评9,流程图10,像素画11
            ...
            string sqlfive = "select * from Consoles where Ncid=" + Cid;
            DataTable ndt = DbHelperSQL.Query(sqlfive).Tables[0];
            int ncount = ndt.Rows.Count;
            if (ncount > 0)
            {
                for (int i = 0; i < ncount; i++)
                {
                    int nid = Int32.Parse(ndt.Rows[i]["Nid"].ToString());
                    string ntitle = ndt.Rows[i]["Ntitle"].ToString();
                    int ncid = Int32.Parse(ndt.Rows[i]["Ncid"].ToString());
                    bool npublish = bool.Parse(ndt.Rows[i]["Npublish"].ToString());
                    lmode.Lcid = ncid;
                    lmode.Lsort = 9;
                    lmode.Ltype = 9;
                    lmode.Lxid = nid;
                    lmode.Lshow = npublish;
                    lmode.Ltitle = ntitle;
                    lbll.Add(lmode);//将所有测评都添加到导航中
                }
            }
        ...
        public void importupsort(DataTable dt, int Cid)
        {
            ...
            if (count > 0)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    //int ltype = Int32.Parse(dt.Rows[i]["Ltype"].ToString());
