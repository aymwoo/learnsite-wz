...
namespace LearnSite.DAL
{
	...
	public partial class TxtFormBack
	{
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetListclass(string Rgrade, string Rclass, string Rmid,int Sgroup,int Sid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Rid,Rmid,Rsnum,Rsid,Rwords,Rtime,Rip,Rscore,Ryear,Rterm,Rgrade,Rclass,Ragree,Sname ");
            strSql.Append(" FROM TxtFormBack,Students where ");
            string strWhere = " Rsid=Sid and Rmid=" + Rmid + " and Sgrade=" + Rgrade + " and Sclass=" + Rclass ;
            if (Sid > 0)
            {
                if (Sgroup > 0)
                {
                    strWhere = strWhere + " and Sgroup=" + Sgroup.ToString();//显示组内
                }
                else {
                    strWhere = strWhere + " and Sid=" + Sid.ToString();//只显示自己
                }
            }
            strSql.Append(strWhere);
            strSql.Append(" order by Rtime asc");
