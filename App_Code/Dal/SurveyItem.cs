...
namespace LearnSite.DAL
{
	...
	public partial class SurveyItem
	{
		...
		public int Add(LearnSite.Model.SurveyItem model)
		{
			...
			strSql.Append("Mqid,Mvid,Mitem,Mscore,Mcount,Mcid,Mblack)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@Mblack", SqlDbType.Bit,1)};
            ...
            parameters[6].Value = model.Mblack;
        ...
        public bool UpdateItemMblack(int Mqid, bool Mblack)
        {
            ...
            strSql.Append("Mblack=@Mblack ");
            strSql.Append(" where Mqid=@Mqid");
            SqlParameter[] parameters = {
					new SqlParameter("@Mqid", SqlDbType.Int,4),
					new SqlParameter("@Mblack", SqlDbType.Bit,1)};
            parameters[0].Value = Mqid;
            parameters[1].Value = Mblack;
		...
		public LearnSite.Model.SurveyItem GetModel(int Mid)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 Mid,Mqid,Mvid,Mitem,Mscore,Mcount,Mcid,Mblack from SurveyItem ");
			strSql.Append(" where Mid=@Mid");
			SqlParameter[] parameters = {
					new SqlParameter("@Mid", SqlDbType.Int,4)
			};
			...
			if(ds.Tables[0].Rows.Count>0)
			{
                ...
                if (ds.Tables[0].Rows[0]["Mblack"] != null)
                {
                    if ((ds.Tables[0].Rows[0]["Mblack"].ToString() == "1") || (ds.Tables[0].Rows[0]["Mblack"].ToString().ToLower() == "true"))
                    {
                        model.Mblack = true;
                    }
                    else
        ...
        public LearnSite.Model.SurveyItem GetModel(DataTable dt, int Tsort)
        {
            ...
            if (Count > 0)
            {
                if (Tsort < Count)
                {
                    ...
                    if (dt.Columns.Contains("Mblack"))
                    {
                        if (dt.Rows[Tsort]["Mblack"] != null)
                        {
                            if ((dt.Rows[Tsort]["Mblack"].ToString() == "1") || (dt.Rows[Tsort]["Mblack"].ToString().ToLower() == "true"))
                            {
                                model.Mblack = true;
                            }
                            else
        ...
        {
            string strSql = "select Mid,Mitem FROM SurveyItem where (Mblack=0 or Mblack is null) and  Mqid=" + Mqid + " order by NewID()";
            return DbHelperSQL.Query(strSql);
        }
        /// <summary>
        /// 获得数据列表Mid,Mitem，顺序排序
        /// </summary>
        public DataSet GetListItemBlack(int Mqid)
        {
            string strSql = "select Mid,Mitem FROM SurveyItem where Mblack=1 and  Mqid=" + Mqid ;
            return DbHelperSQL.Query(strSql);
        }
        /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select Mid,Mqid,Mvid,Mitem,Mscore,Mcount,Mcid,Mblack ");
        ...
        public int GetItemAllScore(int Mvid)
        {
            ...
            strSql.Append(" where Mvid=@Mvid");
            SqlParameter[] parameters = {
					new SqlParameter("@Mvid", SqlDbType.Int,4)};

            parameters[0].Value = Mvid;
        ...
        public int GetItemScore(ArrayList fselect)
        {
            if (fselect.Count > 0)
            {
                string mids = string.Join(",", (string[])fselect.ToArray(typeof(string)));
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select sum(Mscore) FROM SurveyItem ");
                string strWhere = " Mid in (" + mids + ") ";
                if (strWhere.Trim() != "")
                {
                    strSql.Append(" where " + strWhere);
                }
        ...
        {
            string strSql = "select Mitem,Mscore,Mid FROM SurveyItem where Mblack=1 and  Mvid=" + Mvid.ToString();
            DataTable dt = DbHelperSQL.Query(strSql).Tables[0];
            ...
            {
                string answer = bbtext[i].ToString();
                string mitem = dt.Rows[i][0].ToString().Trim();

                if (answer == mitem)
                {
                    int mscore = Int32.Parse(dt.Rows[i][1].ToString());
                    score = score + mscore;
                    temp = temp + answer + "=" + mitem + "\r\n";
                    okmids = okmids + dt.Rows[i][2].ToString() + ",";
                }
                else
                {
                    temp = temp + answer + "!=" + mitem + "\r\n";
            ...
            if (okmids.EndsWith(","))
                okmids = okmids.Substring(0, okmids.Length - 1);
