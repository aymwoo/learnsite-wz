...
namespace LearnSite.DAL
{
	...
	public class TopicReply
	{
		...
		public int Add(LearnSite.Model.TopicReply model)
		{
            ...
            strSql.Append("Rtid,Rsnum,Rwords,Rtime,Rip,Rscore,Rban,Rgrade,Rterm,Rcid,Rclass,Rsid,Ryear,Redit,Ragree,Rlid)");
            ...
            parameters[15].Value = model.Rlid;
        ...
        /// </summary>
        /// <param name="Rid"></param>
        public int UpdateAllscore6(int Rtid, int Rgrade, int Rclass, int Ryear)
        {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  Rid,Rtid,Rsnum,Rwords,Rtime,Rip,Rscore,Rban,Rgrade,Rterm,Redit,Ragree,Sname,Sgroup,Sleader ");
            strSql.Append(" FROM TopicReply,Students ");
            string strWhere = " Rsnum=Snum and Rban=0 and Rtid=" + Rtid + " and Sgrade=" + Sgrade + " and Sclass=" + Sclass + " order by Sgroup desc,Sleader desc, Rtime desc";
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public string ShowStuDoneReplyCids(string Snum, int Rterm, int Rgrade)
        {
            ...
            if (n > 0)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    cidstr = dt.Rows[i]["Rcid"].ToString();
