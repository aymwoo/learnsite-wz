    ...
    public class SqlHelper
    {
        ...
        public static DataSet CourseMissionQuery(int Cid)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                string strProblem = "select  * from Problems where Pcid=" + Cid;
                string strJudgeArg = "select  * from JudgeArg where Jcid=" + Cid;
                try
                {
                    ...
                    SqlDataAdapter dan = new SqlDataAdapter(strConsole, conn);
                    dan.Fill(ds, "Console");
                    SqlDataAdapter dap = new SqlDataAdapter(strProblem, conn);
                    dap.Fill(ds, "Problem");
                    SqlDataAdapter daj = new SqlDataAdapter(strJudgeArg, conn);
                    daj.Fill(ds, "JudgeArg");
        ...
        public static int CountTable() {
            string sqlstr = "SELECT count(TABLE_NAME) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE'";
            object obj = GetSingle(sqlstr);
            if (obj == null)
            {
                return 0;
            }
