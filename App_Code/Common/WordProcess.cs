...
namespace LearnSite.Common
{
    ...
    public class WordProcess
    {
        ...
        /// <param name="Wurl"></param>
        /// <returns></returns>
        public static string officepng(string Wurl)
        {
            string xlspath = HttpContext.Current.Server.MapPath(Wurl);
            string str = Common.Template.officehtm();
            string ext = LearnSite.Common.WordProcess.getext(Wurl);

            string extions = "." + ext;
            string pngurl = Wurl.Replace(extions, ".png");
            string pngpath = HttpContext.Current.Server.MapPath(pngurl);

            if (File.Exists(pngpath))
            {    //如果同时成功，则显示图片
                string jsurl = pngurl.Replace("~", "..");
                str = "<img src='" + jsurl + "' />";
            }
            else
            {
        ...
        /// <param name="Wurl"></param>
        /// <returns></returns>
        public static string officepdf(string Wurl)
        {
            string xlspath = HttpContext.Current.Server.MapPath(Wurl);
            string str;
            string ext = LearnSite.Common.WordProcess.getext(Wurl);
            ...
            string extions = "." + ext;
            string pdfurl = Wurl.Replace(extions, ".pdf");
            string pdfpath = HttpContext.Current.Server.MapPath(pdfurl);

            if (File.Exists(pdfpath))
            {    //如果同时成功，则显示图片
                string str1 = "<iframe src='../plugins/pdf/web/viewer.html?file=";
                string str2 = "' width='100%' height='100%' min-height='500px' id='Iframe'></iframe>";

                //string host ="http://"+ HttpContext.Current.Request.Url.Host;

                str = str1 + pdfurl.Replace("~", "../../..") + str2;
            }
            else
            {
                //采用weboffice控件浏览
        ...
        public static string htmlsite(string photourl,string snum)
        {
            string str = "";
            ...
            {
                string shtm = Common.Template.sitehtm();
                if (shtm != "")
                {
                    string jsurl = photourl.Replace("~", "..");
                    shtm = shtm.Replace("1.png", jsurl);

                    string siteurl = "../website/" + snum + "/index.html";
                    shtm = shtm.Replace("index.html", siteurl);
        ...
        public static string photobaseauto(string photourl)
        {
            string str = "<br/><br/> ~o~ 找不到图片!<br/> ";
            string photopath = HttpContext.Current.Server.MapPath(photourl);
            string base64img = "";
            ...
            if (File.Exists(photopath))
            {
                ...
                if (isPicNew(photopath))
                {
                    ...
                    if (fni.Ext == "psd")
                    {
                        ...
                        if (pWidth > showlimit)
                        {
                            int x = pWidth / showlimit;
                            bmp = Magnifier(bmp, x);
                        }
                        base64img = ToBase64(bmp);
                        bmp.Dispose();
                    }
                    else
                    {
                    ...
                    string style = "height='"+pHeight.ToString()+"' width='"+pWidth.ToString()+"'";
                    string shtm = "<img src='data:image/png;base64," + base64img + "'" + style + " />";
                    str = shtm;
                }
                else
                {
        ...
        /// <returns></returns>
        public static Bitmap Magnifier(Bitmap original, int multiple)
        {
            Bitmap resized = new Bitmap(original, new Size(original.Width / multiple, original.Height / multiple));
            return resized;
        }
        ...
        private static string ToBase64(Bitmap bmp)
        {
            ...
            encoderParameters.Param[0] = new EncoderParameter(System.Drawing.Imaging.Encoder.Quality, 30L);//压缩质量
            bmp.Save(memStream, GetEncoder(ImageFormat.Jpeg), encoderParameters);
        ...
        public static string kmhtml(string mmurl)
        {
            string str = "";
            string shtm = Common.Template.kitymindhtm();
            string mmpath = HttpContext.Current.Server.MapPath(mmurl);
            if (File.Exists(mmpath))
            {
                if (shtm != "")
                {
                    string jsurl = mmurl.Replace("~", "..");
                    str = shtm.Replace("demo.km", jsurl);
                }
            }
            return str;
        }
        ...
        public static string sb3html(string sburl)
        {
            string str = "";
            string shtm = Common.Template.scratchjshtm(); 
            string mmpath = HttpContext.Current.Server.MapPath(sburl);
            if (File.Exists(mmpath))
            {
                if (shtm != "")
                {
                    string jsurl = sburl.Replace("~", "..");
                    str = shtm.Replace("Friend.sb3", jsurl);
                }
            }
            return str;
        }

        public static string sb3htmlcode(string sburl)
        {
            string str = "";
            string shtm = Common.Template.scratchcodehtm();
            string mmpath = HttpContext.Current.Server.MapPath(sburl);
            if (File.Exists(mmpath))
            {
                if (shtm != "")
        ...
        public static string pdfhtml(string pdfurl)
        {
            ...
            string str = str1 +  pdfurl.Replace("~", "../../..") + str2;
        ...
        {
            string str = "";
            string shtm = Common.Template.photohtm(); // Common.Template.scratchhtm();
        ...
        /// <returns></returns>
        public static string txtview(string txturl)
        {
            string str = "没有文本文件";
            string txtpath = HttpContext.Current.Server.MapPath(txturl);
            if (File.Exists(txtpath))
        ...
        /// <param name="txturl"></param>
        /// <returns></returns>
        public static string codeview(string txturl)
        {
            string str = "没有文本文件";
            string txtpath = HttpContext.Current.Server.MapPath(txturl);
            if (File.Exists(txtpath))
            {
                string strTemp = File.ReadAllText(txtpath, EncodingType.GetType(txtpath));
                //strTemp = Base64(strTemp); 
                strTemp = Base64(HttpUtility.UrlEncode(strTemp).Replace("+", "%20"));

                string shtm = Common.Template.codehtm();
                if (shtm != "")
                {
                    str = shtm.Replace("#code#", strTemp);
                }
            }
            return str;
        }
        ...
        /// <param name="txturl"></param>
        /// <returns></returns>
        public static string pixelsmall(string txturl)
        {
            string str = "";
            string txtpath = HttpContext.Current.Server.MapPath(txturl);
            if (File.Exists(txtpath))
            {
                string strTemp = File.ReadAllText(txtpath, EncodingType.GetType(txtpath));
                string shtm = "<table id='pixel_small'>"+strTemp+"</table>";
                str = shtm;
            }
            return str;
        }
        ...
        /// <param name="pyurl"></param>
        /// <returns></returns>
        public static string blockstuview(string wid)
        {
            Model.Works wmodel = new Model.Works();
            BLL.Works bll = new BLL.Works();
            wmodel = bll.GetModel(Int32.Parse(wid));
            ...
            string str = "#没有代码";
            //LearnSite.Common.Log.Addlog("pyBlock", Wcode);
            string codefile = HttpUtility.UrlEncode(Wcode).Replace("+", "%20"); //如果不为空，则获取原来的作品链接
        ...
        /// <param name="pyurl"></param>
        /// <returns></returns>
        public static string pystuview(string wid)
        {
            string str = "没有文本文件";
            Model.Works wmodel = new Model.Works();
            BLL.Works bll = new BLL.Works();
            ...
            string shtm = Common.Template.pythonstuhtm();
            string Wcode = wmodel.Wcode;
            string pyurl = wmodel.Wurl;
            string code = Common.Template.codehtm();

            if (Wcode == "")
            {
                string pypath = HttpContext.Current.Server.MapPath(pyurl);
                string strTemp = File.ReadAllText(pypath, EncodingType.GetType(pypath));
                strTemp = Base64(HttpUtility.UrlEncode(strTemp).Replace("+", "%20"));  
                code = code.Replace("#code#", strTemp);
                return code;
            }
            else
            {
                string img = pyurl.Replace("py", "png");
                img = img.Replace("~", "..");
                str = shtm.Replace("#cover#", img);
                str = str.Replace("#codefile#", Wcode);
                return str;
            }
        }
        /// <summary>
        ...
        /// <param name="pyurl"></param>
        /// <returns></returns>
        public static string pyview(string pyurl)
        {
            string str = "没有文本文件";
            string pypath = HttpContext.Current.Server.MapPath(pyurl);
            ...
            {
                string strTemp = File.ReadAllText(pypath, EncodingType.GetType(pypath));
                //LearnSite.Common.Log.Addlog("读取py文本：", strTemp);
                strTemp = Base64(HttpUtility.UrlEncode(strTemp).Replace("+", "%20"));  
                
                string shtm = Common.Template.pythonhtm();
                if (shtm != "")
                {
                    string img = pyurl.Replace("py", "png");
                    img = img.Replace("~", "..");
                    str = shtm.Replace("#cover#", img);
                    str = str.Replace("#codefile#", strTemp);
        ...
        /// <param name="pyurl"></param>
        /// <returns></returns>
        public static string pystuWcode(string Wcode, string pyurl)
        {
            string str = "没有文本文件";
            ...
            {
                string pypath = HttpContext.Current.Server.MapPath(pyurl);
                string strTemp = File.ReadAllText(pypath, System.Text.Encoding.UTF8);
                strTemp = Base64(HttpUtility.UrlEncode(strTemp).Replace("+", "%20"));  
                code = code.Replace("#code#", strTemp);
                return code;
            }
        ...
        /// <param name="pyurl"></param>
        /// <returns></returns>
        public static string pyBlock(string Wcode)
        {
            string str = "#没有代码";
            //LearnSite.Common.Log.Addlog("pyBlock", Wcode);
            string codefile = HttpUtility.UrlEncode(Wcode).Replace("+", "%20"); //如果不为空，则获取原来的作品链接
        ...
        /// <param name="pyurl"></param>
        /// <returns></returns>
        public static string pyWcode(string Wcode, string pyurl)
        {
            string str = "#没有代码";
            ...
            if (Wcode == "")
            {
                ...
                if (File.Exists(pypath))
                {
                    string strTemp = File.ReadAllText(pypath, Encoding.UTF8);
                    strTemp = Base64(HttpUtility.UrlEncode(strTemp).Replace("+", "%20"));
                    code = code.Replace("#code#", strTemp);
                    return code;
                }
            ...
            else
            {
                string shtm = Common.Template.pythonhtm();
                string img = pyurl.Replace("py", "png");
                img = img.Replace("~", "..") + "?temp=" + DateTime.Now.Millisecond.ToString();
                str = shtm.Replace("#cover#", img);
                str = str.Replace("#codefile#", Wcode);
                return str;
            }
        ...
        /// 根据链接读取文本代码
        /// </summary>
        /// <param name="codeurl">相对链接</param>
        /// <returns></returns>
        public static string readCode(string codeurl) {
            string pypath = HttpContext.Current.Server.MapPath(codeurl);
            string strTemp = File.ReadAllText(pypath, EncodingType.GetType(pypath));
            return strTemp;
        }
        ...
        public static string SelectWrite(string filetype, string downfilename, bool isflex)
        {
            ...
            switch (filetype)
            {
                case "htm":
                case "html":
                    string siteurl = downfilename.Replace("~", "..");
                ...
                case "xls":
                case "xlsx":
                    if (XmlHelp.GetOfficePng())
                        str = officepng(downfilename);
                    else
                ...
                case "mpg":
                    ...
                    break;
                default:
                    str = "<div style='text-align: center;font-size: 11pt;'>该文件格式无法支持在线预览~!</div>";
                    break;
            }
        ...
        public static string SelectWriteNew(string filetype, string downfilename, bool isflex)
        {
            ...
            switch (filetype)
            {
                ...
                case "ino":
                case "lgo":
                    str = txtview(downfilename);
                    break;
                case "py":
                    str = pyview(downfilename);
                    break;
                case "swf":
                    str = flashhtml(downfilename);
                    break;
                ...
                case "mm":
                    ...
                    break;
                case "km":
                    str = kmhtml(downfilename);
                    break;
                case "sb":
                case "sb2":
                case "sb3":
                    str = sb3html(downfilename);//sbhtml(downfilename);
                    break;
                case "dae":
                    str = daehtml(downfilename);
                    break;
                ...
                case "psd":
                case "bmp":
                    str = photobaseauto(downfilename);
                    break;
                case "jpg":
                case "png":
                case "gif":
                ...
                case "svg":
                    ...
                    break;
                case "xml":
                    str = photohtmlauto(downfilename.Replace("xml", "png"));
                    break;
                case "flv":
                case "mp4":
                    str = flvhtml(downfilename);
        ...
        public static string SelectEvaluateShow(string Wid,string filetype, string downfilename, bool isflex)
        {
            ...
            switch (filetype)
            {
                ...
                case "lgo":
                    ...
                    break;
                case "pxl":
                    str = pixelview(downfilename);
                    break;
                case "py":
                    str = pystuview(Wid);
                    break;
                case "block":
                    str = blockstuview(Wid);
                    break;
                case "swf":
                    str = flashhtml(downfilename);
                    break;
        ...
        public static string SelectWriteTeaNew(string filetype, string downfilename, bool isflex,string htmname)
        {
            ...
            string url = "<br /><br /><div style='text-align: center;font-size: 16px; color:#333333;'><a href='" + mydown + "' target='_blank'>下载查看</a>  " + Filesize(downfilename) + "kb</div>";
            str = str + url;
            return str;//返回预览的html生成页面
        }

        public static string SelectWriteTeaShow(string Wid, string filetype, string downfilename, bool isflex, string htmname)
        {
            string str = "";
            switch (filetype)
            {
                ...
                case "sb2":
                case "sb3":
                    str = sb3htmlcode(downfilename);//sbhtml(downfilename);
                    break;
        ...
        public string Base64ToString(string str)
        {
            try
            {
                ...
                str = Encoding.Default.GetString(by);
            }
            catch
            {
            }
