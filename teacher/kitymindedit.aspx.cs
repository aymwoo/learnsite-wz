...
public partial class teacher_kitymindedit : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["mcid"] != null && Request.QueryString["mid"] != null)
        {
            if (!IsPostBack)
            {
                Master.Page.Title = LearnSite.Common.CookieHelp.SetMainPageTitle() + "思维导图修改页面";
                ShowMgid();
                missionview();
            }
        }
        ...
        if (Texttitle.Text != "" && fckstr != "")
        {
            if (Request.QueryString["mcid"] != null && Request.QueryString["mid"] != null)
            {
                ...
                if (Fupload.HasFile)
                {
                    ...
                    mission.Mexample = exampleurl;//如何有上传，则修改思维导图实例
                }
