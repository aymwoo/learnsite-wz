...
public partial class Teacher_graphedit : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["mcid"] != null && Request.QueryString["mid"] != null)
        {
            if (!IsPostBack)
            {
                Master.Page.Title = LearnSite.Common.CookieHelp.SetMainPageTitle() + "流程图修改页面";
                ShowMgid();
                missionview();
            }
        }
        ...
        if (Texttitle.Text != "" && fckstr != "")
        {
            if (Request.QueryString["mcid"] != null && Request.QueryString["mid"] != null)
            {
                ...
                if (Fupload.HasFile)
                {
                    ...
                    mission.Mexample = exampleurl;//如何有上传，则修改编程实例
                }
                ...
                string url = "~/teacher/graphshow.aspx?mcid=" + Mcid + "&mid=" + Mid;
